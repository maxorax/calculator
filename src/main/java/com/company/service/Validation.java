...
public class Validation {
    public static void characterСheck(String expr) throws NotValidCharacterException, NotCorrectExpression {
        for (int i = 0; i < expr.length(); i++) {
            if (expr.charAt(i) != '+' &&
                    ...
                    expr.charAt(i) != '(' &&
                    expr.charAt(i) != '.' &&
                    String.valueOf(expr.charAt(i)).matches("\\D")) {
                throw new NotValidCharacterException("Введен не корректный символ");
            }
        }
        ExpressionCheck.expressionCheck(expr);
    }
